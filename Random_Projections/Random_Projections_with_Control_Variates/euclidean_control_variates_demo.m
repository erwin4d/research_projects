function [rmse] = euclidean_control_variates_demo(X, niter, varargin)
  ...
  % Example inputs
  % euclidean_control_variates_demo(X, 1000, 'option', 'normal', 'opt_para', -1, 'partition', 250);
  ...
  para = get_true_vals_all(X, para, 'squared euclidean distance');
    ...
    for xseg = 1:length(para.partition_start)
      ...
      for yseg = xseg:length(para.partition_start)
        ...
        for kvals = 1:length(kvec)
          ...
          ests.small_euc_dist = get_small_v_obs(ests.euc_dist, kvals, div_by);
          ...
          [ mvn2d_est ] = get_emp_bivar_control_var(ests, currV, kvec, kvals, 'squared_euclidean_distance');


          rmse.ord(iter_num,kvals) = rmse.ord(iter_num,kvals) + get_rmse_all(ests.small_euc_dist, para, xseg, yseg);
...
function [ ] = local_plot(kvec, rmse, iter_num, fig_para)
  ...
  title(['Average RMSE of pairwise Euclidean distances at ', num2str(iter_num), ' iterations'], 'FontWeight', 'bold');
  ...
  title(['3 sds of average RMSE of pairwise Euclidean distances at ', num2str(iter_num), ' iterations'], 'FontWeight', 'bold');
  xlabel('Number of columns K', 'FontWeight', 'bold');
  legend('-DynamicLegend', 'location', 'northeast');
  ylabel('Average RMSE', 'FontWeight', 'bold');
